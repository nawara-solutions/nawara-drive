...
- Splitting/merging a service (e.g. the decision to move `auth`/`notification`/`payment`/`ai`
  into the separate `nawara-core` repo)
- Adopting a monorepo tool, a testing strategy, an auth strategy
- Any decision recorded in [`ARCHITECTURE.md`](../../ARCHITECTURE.md)'s tech stack table that
  needs the _why_, not just the _what_
